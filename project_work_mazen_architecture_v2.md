...
| **ecglib DenseNet-1D** ×7 | Avetisyan 2023 per-pathology AUC ≈ **0.95–0.99**; PTB-XL CNN macro-AUC ≈ 0.92–0.93 | mean ROC-AUC **0.980**, macro F1 0.727 | Yes | Closest — PTB-XL fold 10, **7-pathology subset** | Matches/slightly over — leakage **checked**: macro AUC 0.973 on independent Chapman-Shaoxing-Ningbo (n=1500) ⇒ not inflated |
| **ecglib external check** (Chapman-Shaoxing-Ningbo) | no published number — own leakage probe | **macro AUC 0.973** (n=1500, report-grade) | Yes | No — PTB-XL-independent | **✅ no meaningful leakage** — ≈ the PTB-XL ~0.98 |
...
- **External ECG eval — done, report-grade** (Chapman-Shaoxing-Ningbo): macro AUC 0.973 (n=1500, all records usable; per-pathology AUC 0.90–0.99) ⇒ no meaningful leakage.
...
- **ECG leakage — checked, ruled out** — ecglib's 500 k corpus is unpublished and may include PTB-XL, but on the independent Chapman-Shaoxing-Ningbo set (n=1500) the macro AUC is **0.973** (≈ PTB-XL ~0.98), so the result is not inflated by leakage.
...
| # | Problem | Concrete fix | Effort |
|---|---|---|---|
| 11 | **PTB-XL leakage** ✅ done | Ran `tools/eval_ecg_external.py --stream 1500` on Chapman-Shaoxing-Ningbo: macro AUC **0.973** (n=1500, report-grade; per-pathology AUC 0.90–0.99) ≈ PTB-XL ⇒ no meaningful leakage. | done |
